...
test -f /etc/resolv.conf && echo "/etc/resolv.conf is  regular file" || echo "etc/resolv.conf is not regular file"
# TASK 2: Add a test to see if the /etc/resolv.conf file is a symbolic link
test -L /etc/resolv.conf && echo "/etc/resolv.conf is symbolic link" || echo "/etc/resolv.conf is not symbolic link"
# TASK 3: Add a test to see if the /etc/resolv.conf file is a directory
test -d /etc/resolv.conf && echo "/etc/resolv.conf is directory" || echo "/etc/resolv.conf is not directory"
...
[ -e /tmp ] && echo "/tmp is accessed" || echo "/tmp is not accessed"
